...
public class SchoolClassService {
   ...
   public SchoolClassResponse create(SchoolClassCreateRequest request) {
      ...
      assertSameSchool(year);
      assertSameSchool(grade);
   ...
   private StudyTrack validateTrack(Grade grade, Long studyTrackId) {
      if (grade.isRequiresTrack()) {
         ...
         StudyTrack track = studyTrackRepository.findById(studyTrackId)
                 .orElseThrow(() -> ResourceNotFoundException.of("StudyTrack", studyTrackId));
         assertSameSchool(track);
         return track;
      }
      if (studyTrackId != null) {
         throw new BusinessRuleViolationException(
   ...
   private void assertSameSchool(SchoolClass sc) {
      if (!sc.getSchool().getId().equals(SecurityUtils.currentSchoolId())) {
         throw new AccessDeniedOnResourceException("This class does not belong to your school.");
      }
   }

   private void assertSameSchool(AcademicYear year) {
      if (!year.getSchool().getId().equals(SecurityUtils.currentSchoolId())) {
         throw new AccessDeniedOnResourceException("This academic year does not belong to your school.");
      }
   }

   private void assertSameSchool(Grade grade) {
      if (!grade.getSchool().getId().equals(SecurityUtils.currentSchoolId())) {
         throw new AccessDeniedOnResourceException("This grade does not belong to your school.");
      }
   }

   private void assertSameSchool(StudyTrack track) {
      if (!track.getSchool().getId().equals(SecurityUtils.currentSchoolId())) {
         throw new AccessDeniedOnResourceException("This study track does not belong to your school.");
      }
   }
}